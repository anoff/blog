...
Administration is easiest via [Azure Cloud shell](https://shell.azure.com) after activating _Allow access to Azure services_ in the _Connection Security_ pane of the Postgres server.
...
**📌 NOTE**\
This step is only necessary if you want to use external data disks like I do and do not have them mounted already.
...
**📌 NOTE**\
For the Gitea setup we will later need this users ID.
...
Even though Gitea and Drone come with integrated [Let’s Encrypt](https://letsencrypt.org/) support to generate SSL certificates I chose another path.
...
In this setup [traefik](https://traefik.io/) is used as a low profile router.
...
**📌 NOTE**\
If you do not wish to use a reverse proxy then simply do not follow the traefik specific configuration.
...
services:
  server:
    ...
    environment:
      ...
      - USER_GID=1001
      - DB_TYPE=postgres
      - DB_HOST=mydb.postgres.database.azure.com:5432 ②
      - DB_NAME=gitea
      - DB_USER=gitea
...
See the [howtoforge.com/ubuntu-docker-traefik-proxy](https://www.howtoforge.com/tutorial/ubuntu-docker-traefik-proxy/#step-install-and-configure-traefik-reverse-proxy) blog post for more details.
...
After the initial start of the Gitea container, stop it again `docker-compose stop` and modify the configuration as needed. Refer to the [Gitea Config Cheatsheet](https://docs.gitea.io/en-us/config-cheat-sheet) for a full list of available settings.
...
services:
  drone-server:
    ...
    environment:
      ...
      # DRONE_DEBUG: "true"
      DRONE_SECRET: "somethingverysecret" ①
